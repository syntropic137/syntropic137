...
def _print_op_summary(ctx: SetupContext) -> None:
    ...
    header = (
        ...
        f"#        op signin                              # uses most recent account\n"
        f"#        op signin --account my-team.1password.com  # specific account\n"
        f"#\n"
        f"#      Multiple accounts? op uses the most recently signed-in account.\n"
        f"#      To check or switch:\n"
        f"#        op account list                        # see all accounts\n"
        f"#        op account get --account SHORTHAND     # verify which is active\n"
        f"#        op signin --account SHORTHAND          # switch to a different one\n"
        f"#      Or set OP_ACCOUNT=SHORTHAND in your shell to pin it.\n"
        f"#\n"
