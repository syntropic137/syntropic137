...
def _print_op_summary(ctx: SetupContext) -> None:
    ...
    header = (
        ...
        f"#   2. Sign in: op signin\n"
        f"#      - If you have multiple accounts (personal + business),\n"
        f"#        sign in to the one that owns the vault \"{vault}\":\n"
        f"#          op signin --account my-team.1password.com\n"
        f"#      - List accounts: op account list\n"
