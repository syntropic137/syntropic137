#!/usr/bin/env bash
# Selfhost environment loader — sources infra/.env and resolves 1Password secrets.
# Sourced by justfile selfhost-* recipes to ensure Docker Compose sees all env vars.
#
# After sourcing this script, all variables from infra/.env are exported,
# and any 1Password-managed secrets are resolved into the environment.

# 1. Source infra/.env (export all variables for Docker Compose interpolation)
if [ -f infra/.env ]; then
    set -a
