...
async def test_projection():
    ...
    # Assertions
    assert result['input_tokens'] == 18000, f"Input tokens mismatch: expected 18000, got {result['input_tokens']}"
    assert result['output_tokens'] == 4500, f"Output tokens mismatch: expected 4500, got {result['output_tokens']}"
    assert result['cache_creation_tokens'] == 5000, f"Cache creation mismatch: expected 5000, got {result['cache_creation_tokens']}"
    assert result['cache_read_tokens'] == 21000, f"Cache read mismatch: expected 21000, got {result['cache_read_tokens']}"
    assert result['tool_calls'] == 15, f"Tool calls mismatch: expected 15, got {result['tool_calls']}"
    ...
    print(f"✅ Cost calculation verified: ${result['total_cost_usd']:.4f} matches expected ${expected_cost:.4f}")
