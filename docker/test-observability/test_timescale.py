...
async def test_timescale():
    ...
    conn = await asyncpg.connect(
        host="timescale-test", port=5432, user="test", password="test", database="obs_test"
    )
    ...
    # 6. Query back
    row = await conn.fetchrow("SELECT * FROM test_observations ORDER BY time DESC LIMIT 1")
