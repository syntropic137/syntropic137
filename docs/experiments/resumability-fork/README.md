...
| `test_exp4_cancelled_resume.py` | What does the system actually do when a CANCELLED execution is resumed? | The aggregate refuses all twelve commands on a rehydrated cancelled execution. The HTTP resume controller decides from the detail PROJECTION and never loads the aggregate, returning success on a `paused` row. NOTE the experiment hand-sets that row, which production cannot do - `save_state` is a no-op and `get_state` reads the event-derived projection. The reachable window is projection LAG, not injection. |
