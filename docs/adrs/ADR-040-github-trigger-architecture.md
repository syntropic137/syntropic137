...
Execution tracking piggybacks on `record_fire()` — when a trigger fires, the execution is marked as running. When the workflow completes (via `WorkflowExecutionCompletedEvent` or `WorkflowFailedEvent`), it's marked as done via `complete_execution()`.
