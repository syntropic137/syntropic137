...
### `/workspace/CLAUDE.md` — Synthetic, Injected at Provisioning Time

A `CLAUDE.md` file is now present at the workspace root before the agent starts. It is generated by the Python layer after setup-phase cloning completes and injected via `workspace.inject_files()`. Its content `@`-imports each cloned repo's own `CLAUDE.md`:
...
Claude Code discovers `CLAUDE.md` at launch from the working directory. Because `/workspace/` is the agent's cwd and this file is present at launch, all project context is loaded from turn 1 — no manual reads, no warm-up turns.
...
```
/workspace/
├── CLAUDE.md                ← synthetic, generated at provisioning time
│                              @-imports each repo's CLAUDE.md
├── artifacts/
│   ├── input/               ← previous phase outputs (read-only, populated by framework)
│   └── output/              ← current phase deliverables (agent writes here)
└── repos/
    ├── repo-a/              ← pre-cloned during setup phase
    │   ├── CLAUDE.md        ← imported by /workspace/CLAUDE.md
    │   └── ...
...
| Syn137 `WorkspaceService` | Injects `/workspace/CLAUDE.md` after cloning; injects `artifacts/input/` |
