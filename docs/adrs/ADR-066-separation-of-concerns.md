...
| **Workspace** | `lib/agentic-workspace/providers/workspaces/*` | Isolated agent execution. Has the heavy tooling baked in (claude CLI, git, language servers, hooks). Runs the actual agent. | Talking back to the API mid-task (the orchestrator drives, the workspace executes). |
