...
export function LLMCopyButton({ content, title, editUrl, txtUrl }: { content: string; title: string; editUrl?: string; txtUrl: string }) {
  ...
  return (
    <div className="flex items-center gap-2 mb-4 not-prose">
      <button
        onClick={handleCopy}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border border-zinc-700 hover:border-sky-500/30 text-fd-muted-foreground hover:text-fd-foreground hover:bg-zinc-800/50 transition-all"
      >
