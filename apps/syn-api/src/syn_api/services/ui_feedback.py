...
Callers ask this module what was decided, never how. ``main.py`` mounts
the router unconditionally so the OpenAPI spec is identical in both
states; ``lifecycle.py`` calls :func:`connect` and :func:`disconnect`;
the routes get their storage from :func:`get_feedback_storage`.
"""
...
#
# NOTE: the production gateway does not set client_max_body_size, so nginx's
# own 1 MB default caps uploads lower than this in the packaged deployment.
# See docs/adrs/ADR-016 follow-ups and the PR body — raising it is an infra
# change in a CODEOWNERS path and is deliberately not made here.
