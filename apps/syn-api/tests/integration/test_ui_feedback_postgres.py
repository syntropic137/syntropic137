...
async def test_round_trip_through_the_api(ephemeral_database: str):
    ...
    with configured(enabled=True, db_url=ephemeral_database):
        ...
        try:
            ...
            downloaded = client.get(f"/feedback/{feedback_id}/media/{media_id}")
            assert downloaded.status_code == 200
            assert downloaded.content == png

            # Filterable by the things an agent triages on.
            by_route = client.get("/feedback", params={"route": "/executions/exec-1"}).json()
            ...
            assert client.get("/feedback", params={"type": "bug"}).json()["total"] == 1
            assert (
                client.get("/feedback", params={"created_after": "2999-01-01T00:00:00Z"}).json()[
                    "total"
                ]
                == 0
            )
            ...
            assert client.get("/feedback", params={"status": "open"}).json()["total"] == 0
