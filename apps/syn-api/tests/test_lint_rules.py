...
ROUTES_DIR = Path(__file__).resolve().parents[1] / "src" / "syn_api" / "routes"
...
class TestNoGetattr:
    ...
    def test_no_getattr_in_api_v1(self) -> None:
        """No getattr with string literals in apps/syn-api/src/syn_api/v1/."""
    ...
    def test_no_getattr_in_routes(self) -> None:
        """No getattr with string literals in apps/syn-api/src/syn_api/routes/."""
        violations = _scan_dir(ROUTES_DIR)
        if violations:
            msg_lines = [
                f"Found {len(violations)} getattr() call(s) with string literals in routes.",
                "Use direct attribute access instead (see issue #116):",
                "",
            ]
            for path, line, text in violations[:20]:
