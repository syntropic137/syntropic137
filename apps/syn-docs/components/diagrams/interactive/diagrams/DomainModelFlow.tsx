...
// 5 bounded context columns — wider spacing, uniform groups
const COL_GAP = 160;
const COL1 = 0;
const COL2 = COL1 + COL_GAP;
const COL3 = COL2 + COL_GAP;
const COL4 = COL3 + COL_GAP;
const COL5 = COL4 + COL_GAP;
const GRP_W = 150;
const GRP_H = 230; // uniform height for all groups
const GRP_TOP = 0;
const NODE_X_OFFSET = 5; // center sm nodes (140px) in 150px group

// Bottom pipeline — use layoutRow for even spacing
const PIPELINE_Y = 270;
const PIPELINE_CX = (COL5 + GRP_W) / 2; // center under the groups
...
const nodes: Node[] = [
  ...
  },
  {
    ...
    data: { title: 'Agent Sessions', color: 'purple' },
    style: { width: GRP_W, height: GRP_H },
  },
  {
  ...
  {
    id: 'grpOrg', type: 'groupNode', position: { x: COL5, y: GRP_TOP },
    data: { title: 'Organization', color: 'emerald' },
    style: { width: GRP_W, height: GRP_H },
  },

  // Orchestration nodes — 3 vertically stacked
  { id: 'workflow', type: 'flowNode', position: { x: COL1 + NODE_X_OFFSET, y: 40 }, data: { icon: 'workflow', label: 'WorkflowTemplate', color: 'indigo', size: 'sm' } },
  ...
  // Agent Sessions — 1 node, vertically centered in group
  { id: 'session', type: 'flowNode', position: { x: COL2 + NODE_X_OFFSET, y: 100 }, data: { icon: 'activity', label: 'AgentSession', color: 'purple', size: 'sm' } },

  // GitHub — 2 nodes
  { id: 'installation', type: 'flowNode', position: { x: COL3 + NODE_X_OFFSET, y: 70 }, data: { icon: 'github', label: 'Installation', color: 'pink', size: 'sm' } },
  { id: 'triggers', type: 'flowNode', position: { x: COL3 + NODE_X_OFFSET, y: 140 }, data: { icon: 'zap', label: 'TriggerRule', color: 'pink', size: 'sm' } },

  // Artifacts — 1 node, vertically centered
  { id: 'artifactStore', type: 'flowNode', position: { x: COL4 + NODE_X_OFFSET, y: 100 }, data: { icon: 'drive', label: 'Artifact', color: 'cyan', size: 'sm' } },

  // Organization — 3 nodes vertically stacked
  { id: 'org', type: 'flowNode', position: { x: COL5 + NODE_X_OFFSET, y: 40 }, data: { icon: 'layers', label: 'Organization', color: 'emerald', size: 'sm' } },
  { id: 'system', type: 'flowNode', position: { x: COL5 + NODE_X_OFFSET, y: 110 }, data: { icon: 'server', label: 'System', color: 'emerald', size: 'sm' } },
  { id: 'repo', type: 'flowNode', position: { x: COL5 + NODE_X_OFFSET, y: 180 }, data: { icon: 'git', label: 'Repo', color: 'emerald', size: 'sm' } },
...
export function DomainModelFlow() {
  return <ReactFlowDiagram nodes={nodes} edges={edges} minHeight={380} />;
}
