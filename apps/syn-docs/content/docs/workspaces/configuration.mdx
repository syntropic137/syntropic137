...
The default workspace image is `agentic-workspace-buildfloor`, pinned by digest in `syn-shared`. It is the omni-agent image (both the claude and the codex harness, so switching a phase's `agent.provider` needs no image change) plus a native build floor: a C toolchain, rustup, pnpm and bun.

```bash
# Override via environment variable
SYN_WORKSPACE_DOCKER_IMAGE=ghcr.io/agentparadise/agentic-workspace-buildfloor@sha256:<digest>
```
...
| `GH_REPO` | The primary repo as `owner/repo`, so `gh` resolves a repository without a working tree to infer one from. Absent when the workflow configures no repo, or when the primary one does not parse. See [Workspace Hydration](/docs/workspaces/hydration). |
...
Plugins can declare required environment variables in their `plugin.json`. The orchestrator auto-forwards these from the workspace configuration. See [ADR-033](https://github.com/AgentParadise/agentic-workspace/blob/main/docs/adrs/033-plugin-native-workspace-images.md) for the plugin specification (moved from agentic-primitives, 2026-09, see #1417).
