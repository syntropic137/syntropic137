...
The default workspace image is `omni-agent-workspace`, pinned by digest in `syn-shared`. It hosts both the claude and the codex harness, so switching a phase's `agent.provider` needs no image change.

```bash
# Override via environment variable
SYN_WORKSPACE_DOCKER_IMAGE=ghcr.io/agentparadise/omni-agent-workspace@sha256:<digest>
```
...
| `CODEX_AUTH_JSON` | Codex auth blob, for codex phases (injected file-only during setup) |
