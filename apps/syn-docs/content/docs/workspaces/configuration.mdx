...
The default workspace image is `agentic-workspace-omni-agent`, pinned by digest in `syn-shared`. It hosts both the claude and the codex harness, so switching a phase's `agent.provider` needs no image change.

```bash
# Override via environment variable
SYN_WORKSPACE_DOCKER_IMAGE=ghcr.io/agentparadise/agentic-workspace-omni-agent@sha256:<digest>
```
...
Plugins can declare required environment variables in their `plugin.json`. The orchestrator auto-forwards these from the workspace configuration. See [ADR-033](https://github.com/AgentParadise/agentic-primitives/blob/main/docs/adrs/033-plugin-native-workspace-images.md) for the plugin specification.
