...
| `GH_REPO` | The primary repo as `owner/repo`, so `gh` resolves a repository without a working tree to infer one from. See [Workspace Hydration](/docs/workspaces/hydration). |
