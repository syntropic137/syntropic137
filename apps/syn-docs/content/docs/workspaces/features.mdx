...
<Callout type="info">
RTK, the OTel export, the LSP servers, and the plugin hooks on this page all ship in the `agentic-workspace-claude-cli` image and are consumed by the **claude** harness. The default `omni-agent` image does not carry them, and a phase running on `agent.provider: codex` does not use them on either image. See [Workspace Images](/docs/workspaces).
</Callout>
