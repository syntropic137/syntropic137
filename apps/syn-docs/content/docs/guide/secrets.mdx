...
Syntropic137 stores infrastructure secrets (database, Redis, and MinIO credentials) as file-based secrets with strict permissions, mounted into containers via Docker Compose's secrets mechanism. Application-level API keys (such as `ANTHROPIC_API_KEY` and `CODEX_AUTH_JSON`) are supplied via environment variables or `.env` files and injected only into the processes that need them.
...
A phase's harness credential is the one secret the agent needs at runtime. Which one is injected depends on the phase's `agent.provider`. Both are handled separately from infrastructure secrets:

| Provider | Credential | Notes |
|----------|-----------|-------|
| `claude` | `ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN` | The OAuth token takes priority when both are set |
| `codex` | `CODEX_AUTH_JSON` | The full contents of a Codex `~/.codex/auth.json`. Injected file-only during setup, never passed through argv or logs |
...
Without `CODEX_AUTH_JSON`, a phase declaring `agent.provider: codex` fails to provision. For a deployment credential that a developer running `codex login` on a laptop cannot revoke, use a ChatGPT Business or Enterprise access token: see [Codex access tokens: setup and validation](https://github.com/syntropic137/syntropic137/blob/main/docs/testing/codex-access-token-setup.md).
