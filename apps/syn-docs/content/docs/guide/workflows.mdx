...
phases:
  ...
  - id: review
    ...
    agent:
      provider: codex
      model: gpt-5.6-sol      # name a concrete model id, not an alias
      sandbox: read-only      # codex enforces this, claude does not
```

Running the same work through two vendors is the main reason to mix them. A codex
review phase has no memory of the claude implementation phase beyond the artifacts
it is handed, so it is a genuinely independent read of the work.
...
| `model` | provider model id | harness default | On codex, always name a concrete model id. See [Codex model ids and cost](#codex-model-ids-and-cost). |
| `sandbox` | `read-only`, `workspace-write`, `full-access` | `full-access` | Enforced on codex phases only. See [What `sandbox` actually does](#what-sandbox-actually-does). |
...
`sandbox` is named provider-neutrally, but today it steers `codex exec --sandbox`
and nothing else.

<Callout type="warn">
**Declaring `sandbox: read-only` on a claude phase does not restrict it.** The field
is accepted and recorded, and the claude harness ignores it. Do not read a sandbox
level on a claude phase as a security guarantee. Only codex phases are constrained
at the kernel level.
</Callout>

The two harnesses restrict along different axes and neither guarantee can be
expressed in the other's vocabulary:

| Harness | What it can restrict | What it cannot restrict |
|---------|----------------------|-------------------------|
| `claude` | Which tools exist | Where the process may write |
| `codex` | Where the process may write, enforced by the kernel | Which tools exist. Codex has no tool vocabulary at all. |

Sandbox levels, least to most authority:

| Level | Grants | Use for |
|-------|--------|---------|
| `read-only` | Read and search | Review, verify, and audit phases. A phase that cannot write cannot invent the work it was asked to check. It also cannot write its own deliverable, so pair it with another way to publish. |
| `workspace-write` | Read, write, and run commands inside the workspace | Work that stays inside the container. This level is **not** network-free: egress is available at every level. It also cannot write under `artifacts/output/`, so a phase that publishes an artifact needs `full-access`. |
| `full-access` | Unrestricted filesystem access and network egress | Phases that push a branch or call an external API. Never appropriate for a phase whose job is checking other work. |

### `allowed_tools` and the two harnesses

Setting `allowed_tools` on a **codex** phase is rejected: codex exposes no tool
flag of any kind, and translating a tool allowlist into a sandbox mode would be a
category error, so the platform raises `UnsupportedToolPolicyError` rather than
silently doing something else.

On a **claude** phase, `allowed_tools` is currently inert. It is validated at
authoring time but never reaches the harness, so every claude phase inherits the
full toolset. See
[ADR-069](https://github.com/syntropic137/syntropic137/blob/main/docs/adrs/ADR-069-harness-neutral-phase-definition.md)
for the analysis and the planned fix. Do not rely on it as a restriction today.

### Codex authentication

Codex phases need `CODEX_AUTH_JSON` in your `.env`, holding the full contents of a
Codex `~/.codex/auth.json`. It is injected file-only during workspace setup and
never passed through argv or logs. Without it, a phase with `agent.provider: codex`
fails to provision. Claude phases are unaffected.
...
[Codex access tokens: setup and validation](https://github.com/syntropic137/syntropic137/blob/main/docs/testing/codex-access-token-setup.md).

### Codex model ids and cost

Name a **concrete** model id on codex phases, such as `gpt-5.6-sol` rather than the
`gpt-5.6` alias. Codex does not report its model on the wire, so the platform has
nothing to price the run against if you leave `model` unset. The run then lands
**unpriced** rather than wrongly priced, and no cost for it appears in your reports.
...
| Filesystem sandbox enforcement | No | Yes |
| Hook events (`PreToolUse`, `PostToolUse`) | Yes | No |
| Subagent tracking | Yes | No |
| TodoWrite and the todo projection | Yes | No |
| [Claude plugins](/docs/guide/claude-plugins) (`--plugin-dir`) | Yes | No |

Claude is the richer harness for observability. Codex is the only one that can
enforce a filesystem boundary. Pick per phase accordingly: a codex phase will
simply produce no hook, subagent, or todo data, and your dashboards for that phase
will be correspondingly thinner.

[Skills](/docs/guide/skills) work on both harnesses, because the pinned `skills`
...
<Files>
  <Folder name="my-package" defaultOpen>
    <File name="workflow.yaml" />
    <Folder name="phases" defaultOpen>
...
<Files>
  <Folder name="my-marketplace" defaultOpen>
    <File name="marketplace.json" />
    <Folder name="plugins" defaultOpen>
      <Folder name="research-toolkit" defaultOpen>
        <File name="syntropic137-plugin.json" />
        <Folder name="workflows" defaultOpen>
          <Folder name="deep-research">
            <File name="workflow.yaml" />
            <Folder name="phases">
              <File name="*.md" />
            </Folder>
          </Folder>
          <Folder name="quick-scan">
            <File name="workflow.yaml" />
            <Folder name="phases">
              <File name="*.md" />
            </Folder>
        ...
        <Folder name="phase-library">
          <File name="summarize.md" />
        </Folder>
      </Folder>
      <Folder name="pr-automation">
        <File name="workflow.yaml" />
        <Folder name="phases">
          <File name="*.md" />
        </Folder>
