...
| Harness | What it could restrict in principle | What it cannot restrict | Enforced today |
|---------|-------------------------------------|-------------------------|----------------|
| `claude` | Which tools exist | Where the process may write | No. See the `allowed_tools` section below. |
| `codex` | Where the process may write, enforced by the kernel | Which tools exist. Codex has no tool vocabulary at all. | Yes, via `sandbox`. |

Read that last column carefully. Codex is the only harness enforcing anything at
...
| `full-access` | Unrestricted filesystem access | Phases that publish an artifact, because `workspace-write` cannot write under `artifacts/output/`. Never appropriate for a phase whose job is checking other work. Note this is not the level you need for network access, which every level already has. |
...
Per [ADR-069](https://github.com/syntropic137/syntropic137/blob/main/docs/adrs/ADR-069-harness-neutral-phase-definition.md),
...
If you need to bound what a phase can do, use `sandbox` on a codex phase. That
is the only enforced control available today.
