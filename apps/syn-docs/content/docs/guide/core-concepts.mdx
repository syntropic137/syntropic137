...
2. **Agent Phase**: The agent runs with only its harness credential in its environment: `ANTHROPIC_API_KEY` (or `CLAUDE_CODE_OAUTH_TOKEN`) for a claude phase, `CODEX_AUTH_JSON` for a codex phase. Git operations use cached credentials from the setup phase. Network egress goes through an Envoy sidecar proxy.
