...
|-----------------|-------------|
| `model` | Model id for the phase's harness (`sonnet`, `opus`, `haiku` on claude, `gpt-sol` or a concrete id such as `gpt-6-sol` on codex). Defaults to `opus` on claude and `gpt-sol` on codex |
| `argument-hint` | Shown to users when prompted for input |
| `allowed-tools` | Comma-separated list of permitted tools. Validated at authoring time, then inert: it restricts nothing on either harness today. See [Choosing an Agent Harness](/docs/guide/workflows#allowed_tools-and-the-two-harnesses) |
