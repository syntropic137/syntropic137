...
<FeatureGrid>
  <FeatureCard
    ...
    description="Multi-phase pipelines defined in YAML. Research, plan, implement, review: every workflow runs the same way, every time, on Claude Code or OpenAI Codex."
    gradient="indigo"
  />
  <FeatureCard
    icon="layers"
  ...
  <FeatureCard
    ...
    description="Event triggers enable self-healing CI, auto-responses to review comments, and PR-driven workflows. Works zero-config, no tunnel required."
    gradient="cyan"
  />
  <FeatureCard
    icon="shield"
    title="Workspace Isolation"
    description="Every agent runs in an ephemeral Docker container. setup credentials are cleared before the agent starts. Egress proxies control outbound traffic."
    gradient="green"
  />
</FeatureGrid>
...
When you run a coding agent in your terminal, you get results, but you lose everything else:
...
- **No second opinion**: reviewing work with a different vendor's model means running it by hand
...
| **Harness** | Whichever CLI you happen to have open | Claude Code or OpenAI Codex, chosen per phase in the workflow |
