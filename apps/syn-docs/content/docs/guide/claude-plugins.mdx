...
normalizes all three to a single `(source_url, version)` lock key.
...
The verbose form lets you override the display name. Identity remains
`(source_url, version)` so the same plugin declared at workflow and phase
scope deduplicates cleanly.
...
# workflow that pinned the same (source_url, version) keeps resolving.
...
| `DEV__WORKFLOW_FAIL_ON_PLUGIN_FETCH` | `true` | When seeding workflows from disk, fail boot on a bad plugin reference. Set to `false` in dev/CI to log and skip. |
