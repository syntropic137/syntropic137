...
function CostHeader({ cost }: { cost: ExecutionCost }) {
  return (
...
function CostBreakdownRow({ cost }: { cost: ExecutionCost }) {
  return (
...
function PhaseBreakdown({ costByPhase, totalCostUsd }: { costByPhase: Record<string, string>; totalCostUsd: number }) {
  return (
    <div>
      ...
      <div className="space-y-2">
        {Object.entries(costByPhase).map(([phase, phaseCost]) => {
          const costValue = parseFloat(phaseCost)
          const percentage = totalCostUsd > 0
            ? (costValue / totalCostUsd) * 100
            : 0
...
export function ExecutionCostSummary({ cost, showBreakdown = true }: ExecutionCostSummaryProps) {
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)]">
      ...
      <CostBreakdownRow cost={cost} />
      <MetricsGrid cost={cost} />
      ...
      {showBreakdown && (hasPhaseBreakdown || hasModelBreakdown) && (
        <div className="p-4 space-y-4">
          {hasPhaseBreakdown && (
            <PhaseBreakdown costByPhase={cost.cost_by_phase} totalCostUsd={cost.total_cost_usd} />
          )}
          {hasModelBreakdown && (
            <ModelBreakdown costByModel={cost.cost_by_model} />
          )}
        </div>
      )}
