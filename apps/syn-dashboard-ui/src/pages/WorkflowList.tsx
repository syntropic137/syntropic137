...
function WorkflowRow({ workflow, idx }: { workflow: WorkflowSummary; idx: number }) {
  return (
    <Link
              ...
              <div className="flex items-center gap-3">
                ...
                {workflow.runs_count > 0 && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-surface-elevated)] px-2 py-0.5 text-xs text-[var(--color-text-secondary)]">
                    <Play className="h-3 w-3" />{workflow.runs_count} runs
                  </span>
                )}
              </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--color-text-secondary)]">Reusable workflow definitions you can execute</p>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            ...
            description={searchQuery ? 'Try adjusting your search query' : 'Seed workflows with `just seed-workflows`'}
          />
        </Card>
      ) : (
        <>
