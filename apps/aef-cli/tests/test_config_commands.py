...
from aef_api.types import ConfigIssue, ConfigSnapshot, Ok
...
class TestConfigShow:
    def test_show_config(self) -> None:
        ...
        with patch("aef_api.v1.config.get_config", new_callable=AsyncMock) as mock:
...
class TestConfigValidate:
    def test_validate_no_issues(self) -> None:
        with patch("aef_api.v1.config.validate_config", new_callable=AsyncMock) as mock:
...
class TestConfigEnv:
    def test_env_template(self) -> None:
        with patch("aef_api.v1.config.get_env_template", new_callable=AsyncMock) as mock:
