...
from syn_api.types import Ok, TriggerSummary
...
class TestTriggerList:
    def test_list_empty(self) -> None:
        with patch("syn_api.v1.triggers.list_triggers", new_callable=AsyncMock) as mock:
...
class TestTriggerRegister:
    def test_register_success(self) -> None:
        with patch("syn_api.v1.triggers.register_trigger", new_callable=AsyncMock) as mock:
...
class TestTriggerPauseResume:
    def test_pause_success(self) -> None:
        with patch("syn_api.v1.triggers.pause_trigger", new_callable=AsyncMock) as mock:
    ...
    def test_resume_success(self) -> None:
        with patch("syn_api.v1.triggers.resume_trigger", new_callable=AsyncMock) as mock:
            mock.return_value = Ok(None)
...
class TestTriggerDelete:
    def test_delete_success(self) -> None:
        with patch("syn_api.v1.triggers.delete_trigger", new_callable=AsyncMock) as mock:
            mock.return_value = Ok(None)
...
class TestTriggerDisable:
    def test_disable_none(self) -> None:
        with patch("syn_api.v1.triggers.disable_triggers", new_callable=AsyncMock) as mock:
