...
- **Full observability**: token usage, tool traces, costs, and errors captured via event sourcing for both harnesses. Conversation logs are saved after each session for reviewability, and git hooks capture all git-related events. [Claude Code hooks](https://docs.anthropic.com/en/docs/claude-code/hooks) add task, subagent, and hook-level detail on Claude phases.
