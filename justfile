    ...
    if echo "{{flags}}" | grep -q -- "--1password"; then
        ...
        if [ -n "$_HAS_VALUES" ]; then
            _OP_SCRIPT=".op-save-secrets.sh"
            {
                ...
                echo ""
                echo "# --- Clean secret values from .env (keep keys with empty values) ---"
                echo "echo '🧹 Cleaning secret values from .env...'"
                echo "_SECRETS_TO_CLEAR=(SYN_GITHUB_PRIVATE_KEY SYN_GITHUB_WEBHOOK_SECRET ANTHROPIC_API_KEY CLAUDE_CODE_OAUTH_TOKEN CLOUDFLARE_TUNNEL_TOKEN)"
                echo "for _KEY in \"\${_SECRETS_TO_CLEAR[@]}\"; do"
                echo "  if grep -q \"^\${_KEY}=\" .env 2>/dev/null; then"
                echo "    sed -i '' \"s|^\${_KEY}=.*|\${_KEY}=|\" .env"
                echo "    echo \"   cleared: \${_KEY}\""
                echo "  fi"
                echo "done"
                echo "echo ''"
