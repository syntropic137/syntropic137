    ...
    if echo "{{flags}}" | grep -q -- "--1password"; then
        ...
        if [ -n "$_HAS_VALUES" ]; then
            _OP_SCRIPT=".op-save-secrets.sh"
            {
                ...
                echo "#      - If you have multiple accounts (personal + business),"
                echo "#        sign in to the one that owns the vault \"${_VAULT}\":"
                echo "#          op signin --account my-team.1password.com"
                echo "#      - List accounts: op account list"
