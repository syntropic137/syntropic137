    ...
    if echo "{{flags}}" | grep -q -- "--1password"; then
        ...
        if [ -n "$_HAS_VALUES" ]; then
            _OP_SCRIPT=".op-save-secrets.sh"
            {
                ...
                echo "#        op signin                              # uses most recent account"
                echo "#        op signin --account my-team.1password.com  # specific account"
                echo "#"
                echo "#      Multiple accounts? op uses the most recently signed-in account."
                echo "#      To check or switch:"
                echo "#        op account list                        # see all accounts"
                echo "#        op account get --account SHORTHAND     # verify which is active"
                echo "#        op signin --account SHORTHAND          # switch to a different one"
                echo "#      Or set OP_ACCOUNT=SHORTHAND in your shell to pin it."
                echo "#"
                echo "#   3. The vault \"${_VAULT}\" must exist. Create it in the"
                echo "#      1Password app or: op vault create \"${_VAULT}\""
                echo "#"
                echo "# AFTER RUNNING"
                echo "#   1. Remove secret VALUES from .env (keep keys with empty values):"
                echo "#      SYN_GITHUB_PRIVATE_KEY, SYN_GITHUB_WEBHOOK_SECRET,"
                echo "#      ANTHROPIC_API_KEY, CLAUDE_CODE_OAUTH_TOKEN"
                echo "#   2. Keep OP_SERVICE_ACCOUNT_TOKEN_SYN137_DEV in .env (resolver needs it)"
                echo "#   3. Non-secret config (APP_ID, APP_NAME) can stay in .env"
                echo "#   4. On next startup, 1Password auto-injects secrets"
                echo "#   5. New machine? just onboard-dev --1password picks everything up."
                echo ""
                ...
                echo "echo '   Now clean up:'"
                echo "echo '   1. Remove secret values from .env (keep keys empty)'"
                echo "echo '   2. Delete this script: rm ${_OP_SCRIPT}'"
            } > "$_OP_SCRIPT"
