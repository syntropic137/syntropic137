...
class TestWorkspaceSettings:
    ...
    def test_isolation_backend_override(self) -> None:
        ...
        env = {
            "SYN_WORKSPACE_ISOLATION_BACKEND": "docker_hardened",
        }
        with patch.dict(os.environ, env, clear=True):
            settings = WorkspaceSettings(_env_file=None)
    ...
    def test_cloud_settings_override(self) -> None:
        ...
        env = {
            "SYN_WORKSPACE_CLOUD_PROVIDER": "modal",
            "SYN_WORKSPACE_CLOUD_API_KEY": "secret-key",
            "SYN_WORKSPACE_CLOUD_TEMPLATE": "custom-template",
        }
        with patch.dict(os.environ, env, clear=True):
            settings = WorkspaceSettings(_env_file=None)
...
class TestGitIdentitySettings:
    ...
    def test_default_values(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            # NOTE: github_app_* fields moved to GitHubAppSettings (SYN_GITHUB_* prefix)
    ...
    def test_identity_from_env(self) -> None:
        ...
        env = {
            "SYN_GIT_USER_NAME": "syn-bot[bot]",
            "SYN_GIT_USER_EMAIL": "bot@syntropic137.com",
        }
        with patch.dict(os.environ, env, clear=True):
            git = GitIdentitySettings(_env_file=None)

            assert git.user_name == "syn-bot[bot]"
            assert git.user_email == "bot@syntropic137.com"
    ...
    def test_https_credentials(self) -> None:
        ...
        env = {
            "SYN_GIT_USER_NAME": "test-user",
            "SYN_GIT_USER_EMAIL": "test@example.com",
    ...
    def test_github_app_credentials(self) -> None:
        """GitHub App (SYN_GITHUB_*) should be preferred over HTTPS token."""
    ...
    def test_settings_integration(self) -> None:
        ...
        env = {
            "SYN_GIT_USER_NAME": "agent",
            "SYN_GIT_USER_EMAIL": "agent@syntropic137.com",
        }
        with patch.dict(os.environ, env, clear=True):
            git_identity = GitIdentitySettings(_env_file=None)

            assert git_identity.user_name == "agent"
            assert git_identity.user_email == "agent@syntropic137.com"
...
class TestContainerLoggingSettings:
    ...
    def test_environment_override(self) -> None:
        """Environment variables should override defaults."""
        env = {
            "SYN_LOGGING_LEVEL": "DEBUG",
            "SYN_LOGGING_FORMAT": "text",
            "SYN_LOGGING_LOG_API_CALLS": "true",
        }
        with patch.dict(os.environ, env, clear=True):
            logging = ContainerLoggingSettings(_env_file=None)
    ...
    def test_redaction_disabled(self) -> None:
        ...
        env = {
            "SYN_LOGGING_REDACT_SECRETS": "false",
        }
        with patch.dict(os.environ, env, clear=True):
            logging = ContainerLoggingSettings(_env_file=None)
...
class TestGitIdentityResolver:
    ...
    def test_workflow_override_takes_precedence(self) -> None:
        ...
        env = {
            "SYN_GIT_USER_NAME": "env-user",
            "SYN_GIT_USER_EMAIL": "env@example.com",
        }
