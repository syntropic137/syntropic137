...
from syn_shared.settings import AppEnvironment, Settings, get_settings, reset_settings
...
class TestSettings:
    ...
    def test_database_url_validation(self) -> None:
        ...
        env = {
            "SYN_OBSERVABILITY_DB_URL": "postgresql://user:pass@localhost:5432/db",
            "ESP_EVENT_STORE_DB_URL": "postgresql://user:pass@localhost:5432/db",
        }
        with patch.dict(os.environ, env, clear=True):
            settings = Settings()
            assert settings.syn_observability_db_url is not None
            assert "postgresql" in str(settings.syn_observability_db_url)
...
class TestComputedProperties:
    ...
    def test_use_in_memory_storage(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {"APP_ENVIRONMENT": "test", "SYN_OBSERVABILITY_DB_URL": "postgresql://localhost/db"},
            clear=True,
        ):
            settings = Settings(_env_file=None)
            assert settings.use_in_memory_storage is False
