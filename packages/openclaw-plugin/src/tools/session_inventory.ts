...
function validatePage(args: SessionInventoryArgs): string | null {
  ...
  if (!validBound(args.after, -1)) return "after must be an integer >= -1";
...
export async function synGetSessionInventory(
  ...
  if (args.snapshot_id === undefined) {
    if (args.kind !== undefined || args.after !== undefined || args.limit !== undefined) {
  ...
  const result = await client.get<unknown>(`${base}/${args.snapshot_id}/${kind}`, {
    after: String(after), limit: String(limit),
  });
  return result.ok ? { content: JSON.stringify(result.data) } : formatError(result.error);
}
...
export const sessionInventoryToolDefs = [{
  ...
  inputSchema: {
    ...
    properties: {
      ...
      after: { type: "integer", minimum: -1, description: "Previous page next_after; first page uses -1" },
