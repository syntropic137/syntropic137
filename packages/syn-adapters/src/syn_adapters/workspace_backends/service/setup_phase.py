...
async def _staged_credential_state(ws: ManagedWorkspace) -> _CredentialState:
    ...
    if probe.exit_code != 0:
        logger.error(
            ...
            ws.workspace_id,
            describe_exit_code(probe.exit_code),
            probe.stderr,
        )
...
async def _remove_staged_credential(ws: ManagedWorkspace) -> _GuardOutcome:
    ...
    for attempt in range(1, _MAX_ATTEMPTS + 1):
        ...
        attempts.append(f"exit={describe_exit_code(removal.exit_code)}")
        ...
        logger.warning(
            ...
            attempt,
            _MAX_ATTEMPTS,
            describe_exit_code(removal.exit_code),
            removal.stderr,
        )
...
"""
    ...
    if cleanup.exit_code != 0:
        logger.warning(
            ...
            describe_exit_code(cleanup.exit_code),
            ws.workspace_id,
            cleanup.stderr,
        )
