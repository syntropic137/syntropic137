...
- InMemoryArtifactStorage: Unit tests ONLY (requires SYN_ENVIRONMENT='test')

Usage:
    from syn_adapters.storage.artifact_storage import get_artifact_storage

    # Get configured storage (based on SYN_STORAGE_PROVIDER)
...
For tests:
    ...
    from syn_adapters.storage.artifact_storage import get_test_artifact_storage
    storage = get_test_artifact_storage()

See ADR-012: Artifact Storage Architecture
"""

from syn_adapters.storage.artifact_storage.factory import (
    get_artifact_storage,
    get_test_artifact_storage,
    reset_artifact_storage,
)
from syn_adapters.storage.artifact_storage.memory import (
    ArtifactNotFoundError,
    InMemoryArtifactStorage,
    StorageResult,
    TestOnlyAdapterError,
)
from syn_adapters.storage.artifact_storage.minio import (
