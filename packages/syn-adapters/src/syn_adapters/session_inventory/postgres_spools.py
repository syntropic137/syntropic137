...
class PostgresCaptureSpools:
    ...
    async def settle_run(self, run: RunIdentity) -> None:
        """Idempotent and replay-safe: settles every still-unsettled spool of a
        terminal run, keeping any earlier per-session settlement time."""
        if run.source_instance_id != self._source:
            raise ValueError("Run belongs to another installation")
        async with self._pool.acquire() as conn:
            await conn.execute(
                """UPDATE session_capture_spools SET settled_at=now()
