...
class TestGetSystemActivityHandler:
    ...
    async def test_returns_correlated_executions(self) -> None:
        ...
        await store.save("workflow_executions", "exec-1", {
            "workflow_execution_id": "exec-1", "workflow_name": "Deploy",
    ...
    async def test_filters_out_unrelated_repos(self) -> None:
        ...
        await store.save("repo_correlation", "exec-1:other/repo", {
            "repo_full_name": "other/repo", "execution_id": "exec-1",
        })
        await store.save("workflow_executions", "exec-1", {
    ...
    async def test_pagination(self) -> None:
        ...
        result = await handler.handle(
            GetSystemActivityQuery(system_id="sys-1", limit=2, offset=1)
        )
        assert len(result) == 2
