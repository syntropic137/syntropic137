...
class TestGetSystemPatternsHandler:
    ...
    async def test_groups_failures_by_error(self) -> None:
        ...
        await store.save("workflow_executions", "exec-1", {
            "workflow_execution_id": "exec-1", "status": "failed",
            "error_type": "timeout", "error_message": "Timed out after 300s",
    ...
    async def test_detects_cost_outliers(self) -> None:
        ...
        await store.save("repo_cost", "acme/api", {
            "total_cost_usd": "100.00", "total_tokens": 0,
            "execution_count": 1,
        })
        await store.save("repo_cost", "acme/worker", {
            "total_cost_usd": "1.00", "total_tokens": 0,
            "execution_count": 1,
        })
        await store.save("repo_cost", "acme/web", {
            "total_cost_usd": "1.00", "total_tokens": 0,
            "execution_count": 1,
        })
