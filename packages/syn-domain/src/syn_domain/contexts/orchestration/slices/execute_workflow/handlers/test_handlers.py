...
class TestWorkspaceProvisionHandler:
    ...
    async def test_handle_setup_failure_cleans_up_workspace(self) -> None:
        ...
        with patch("syn_adapters.workspace_backends.service.SetupPhaseSecrets") as MockSecrets:
            ...
            # Both halves of the merged message: WHICH setup and phase (#1236),
            # and the exit status leading rather than stderr (#1158).
            with pytest.raises(
                RuntimeError, match="Secret-injection setup for phase 'Test Phase' exited 1"
            ):
                await handler.handle(
                    todo=todo,
                    phase=phase,
