...
class SavedWork:
    ...
    report ``completed``. A phase killed at its timeout, or an execution the
    user cancelled, has already failed for a reason of its own, and an
    exception thrown while saving its work would replace that reason with this
    one. So the same walk reports here as a value.
    ...
    def is_worth_reporting(self) -> bool:
        ...
        False is the common case and must stay silent: a failing phase that
        was holding nothing would otherwise be handed a paragraph about
        quarantine refs that do not exist, which sends an operator looking for
        work nobody lost.
        """
...
#: early)``. There is no ``(False, False)`` entry: nothing found and nothing
#: unreadable is a workspace that was holding nothing, and
...
_SAVED_HEADLINE: Final[dict[tuple[bool, bool], str]] = {
    ...
    (False, True): (
        "  WHETHER THIS WORKSPACE WAS HOLDING ANYTHING IS UNKNOWN: the "
        "attempt to save it stopped before any repository was read "
        "({unreadable}). That is not a report that nothing was lost, it is "
        "the absence of one."
    ),
}
...
def describe_saved_work(work: SavedWork) -> str:
    ...
    Appended to the failure's or cancellation's own reason rather than
