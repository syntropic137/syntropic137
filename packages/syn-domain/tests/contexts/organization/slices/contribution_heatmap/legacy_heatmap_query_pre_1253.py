"""The contribution heatmap EXACTLY as it read agent_events before #1253.
...
as it stood on `main` at 3812c9e5~1, with the enclosing `CANONICAL_SESSION_USAGE_CTE`
of that same revision inlined below - the live one no longer defines
`session_start`, and importing it would quietly make this file the NEW
behaviour wearing the old name, which is the one thing it must never be.

It exists so test_heatmap_rollup_equivalence.py can run the old
implementation and the new one against the same database and compare their
output. The old implementation IS the specification of what the heatmap
means; #1253 was a change of cost, and a change of cost that changes an
answer is a bug. Nothing but a real database can tell the two apart now,
because the new answers come out of trigger and backfill SQL.

Two rules follow from that, and they matter more than they look:

  * If this file ever needs to change to keep the tests passing, the change
    under test altered a RESULT. Do not edit this file - explain the result.
  * Only the class was renamed. Every SQL string, every Python branch and
    every comment is as it was, including the comments #1253 later proved
    wrong about cost. They are part of the record.
"""
...
# Frozen copy of CANONICAL_SESSION_USAGE_CTE as of 3812c9e5~1. The live
# constant of that name no longer defines `session_start`; #1253 moved that
# out of it. Inlined so this module keeps asking the OLD question.
...
class LegacyTimescaleHeatmapQuery:
