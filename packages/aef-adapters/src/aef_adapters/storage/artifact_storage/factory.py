...
Selects storage adapter based on AEF_STORAGE_PROVIDER environment variable:
- 'minio': Uses MinioArtifactStorage (default for development)
- 'local': Uses LocalArtifactStorage (filesystem)
- For tests: InMemoryArtifactStorage (requires AEF_ENVIRONMENT='test')
...
if TYPE_CHECKING:
    from aef_adapters.storage.artifact_storage.memory import InMemoryArtifactStorage
    from aef_adapters.storage.artifact_storage.minio import MinioArtifactStorage
...
def _get_artifact_storage_instance() -> MinioArtifactStorage:
    ...
    if storage_settings.provider == StorageProvider.LOCAL:
        ...
        msg = (
            ...
            "Use AEF_STORAGE_PROVIDER=minio with MinIO for development."
        )
...
def get_test_artifact_storage() -> InMemoryArtifactStorage:
    ...
    CRITICAL: Only works when AEF_ENVIRONMENT='test'.
