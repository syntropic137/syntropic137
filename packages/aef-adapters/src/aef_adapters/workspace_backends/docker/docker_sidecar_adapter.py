...
Architecture:
    ┌─────────────────────────────────────────────────────┐
    │  Docker Network (aef-workspace-net)                  │
    │                                                      │
    │  ┌──────────────┐      ┌──────────────┐             │
...
DEFAULT_SIDECAR_IMAGE = "aef-sidecar-proxy:latest"
...
class DockerSidecarAdapter:
    ...
    async def start(
        ...
        container_name = f"aef-sidecar-{config.workspace_id[:8]}-{short_id}"
        ...
        env_vars = [
            f"AEF_WORKSPACE_ID={config.workspace_id}",
            f"AEF_TOKEN_SERVICE_URL={self._token_service_url}",
            f"AEF_ALLOWED_HOSTS={','.join(config.allowed_hosts)}",
            f"AEF_LISTEN_PORT={config.listen_port}",
        ]
