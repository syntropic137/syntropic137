...
```bash
uv pip install -e packages/aef-tokens/
```
...
```python
from aef_tokens import TokenVendingService, TokenScope, TokenType
...
```python
from aef_tokens import SpendTracker, WorkflowType
...
from aef_tokens.vending import RedisTokenStore, configure_redis
from aef_tokens.spend import RedisBudgetStore, configure_redis_spend_tracker
...
```bash
# Run all tests
uv run pytest packages/aef-tokens/tests/ -v
