...
class ThroughputBenchmark(BaseBenchmark):
    ...
    async def run(
        ...
        **_kwargs: Any,  # noqa: ANN401
        ...
        while time.perf_counter() < end_time:
            ...
            completed_count += self._collect_task_results(done, result)
        ...
        self._finalize(result, completed_count, start_time, max_concurrent)
        return result
    ...
    def _collect_task_results(
        ...
        """Harvest completed tasks into *result*, returning success count."""
        count = 0
        for task in done:
            try:
                result.add_timing(task.result())
                count += 1
    ...
    def _finalize(
        ...
        """Compute throughput metrics and mark *result* complete."""
