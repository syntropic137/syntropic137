...
Usage:
    ...
    from aef_domain.contexts.github._shared.github_client import GitHubAppClient
...
class GitHubAppClient:
    ...
    def __init__(self, settings: GitHubAppSettings) -> None:
        ...
        if not settings.is_configured:
            raise ValueError(
                "GitHub App not configured. Set AEF_GITHUB_APP_ID, "
                "AEF_GITHUB_PRIVATE_KEY, and AEF_GITHUB_INSTALLATION_ID."
            )
    ...
    async def verify_webhook_signature(self, payload: bytes, signature: str) -> bool:
        ...
        if not secret:
            raise ValueError(
                "Webhook secret not configured: set AEF_GITHUB_WEBHOOK_SECRET environment variable"
            )
